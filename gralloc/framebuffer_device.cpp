...
int init_frame_buffer_locked(struct private_module_t *module)
{
	...
	{
		ALOGD("Nothing to do, already initialized");
	...
	if (property_get_bool("ro.disp.resolution_switch", false))
	{
		if (property_get_int32("persist.disp.resolution", DISPLAY_LOW_POWER_LEVEL_FHD) == DISPLAY_LOW_POWER_LEVEL_HD)
		{
			int flag = DISPLAY_LOW_POWER_LEVEL_HD;
			if (ioctl(fd, K3FB_RESOLUTION_SWITCH, &flag) >= 0)
			{
				ioctl(fd, K3FB_RESOLUTION_UPDATE, &flag);
				property_set("ro.sf.lcd_density", "300");
			}
		}
		if (!property_get_int32("ro.sf.lcd_density", 0))
	...
	/*
	 * Request NUM_FB_BUFFERS screens (at lest 2 for page flipping)
	 */
	...
	if (int(info.width) <= 0 || int(info.height) <= 0)
	{
		...
		// default to 320 dpi
		info.width  = ((info.xres * 25.4f) / 320.0f + 0.5f);
		info.height = ((info.yres * 25.4f) / 320.0f + 0.5f);
	}
	...
	AINF("pixclock     = %u\n"
	     ...
	     "hsync_len    = %u\n"
	     "vsync_len    = %u\n",
	    info.pixclock, info.left_margin,
	    info.right_margin, info.upper_margin,
	    info.lower_margin, info.hsync_len,
	    info.vsync_len);
...
static int init_frame_buffer(struct private_module_t *module)
{
	...
	err = init_frame_buffer_locked(module);
