...
#ifdef MP4DEC_TRACE
#define MP4_API_TRC(str)   ALOGV(str)
#else
...
#define MP4_SET_BOT_ADDR(idx) \
        if (pDecCont->dpbMode == DEC_DPB_DEFAULT) \
        { \
        ...
        { \
            u32 frameSize = (pDecCont->VopDesc.vopWidth * \
                                    pDecCont->VopDesc. \
                                    vopHeight) << 8; \
            pDecCont->ppControl.bottomBusLuma = \
                MP4DecResolveBus(pDecCont, idx) + \
...
#define DEC_VOPD ((DecContainer *)decInst)->VopDesc
    ...
    {
        ALOGE("MP44DecDecode# PARAM_ERROR\n");
    ...
    {
        ALOGE("MP4DecDecode: MP4DEC_FORMAT_NOT_SUPPORTED (line %d)", __LINE__);
    ...
    if (API_STOR.DecStat == UNINIT)
    {
        ...
        ALOGE("MP4DecDecode: MP4DEC_NOT_INITIALIZED (line %d)", __LINE__);
    ...
    {
        ALOGE("MP4DecDecode: MP4DEC_PARAM_ERROR (line %d)", __LINE__);
        return MP4DEC_PARAM_ERROR;
    }
       ...
       pDecCont->StrmStorage.customStrmHeaders)
    {
        ...
        {
            ALOGE("MP4DecDecode: MEMFAIL (%d) (line %d)", internalRet, __LINE__);
            return (internalRet);
        }
    }
    ...
    if (pDecCont->StrmStorage.customStrmHeaders)
        pDecCont->dpbMode = DEC_DPB_DEFAULT; 
 ...
 */
    ...
    do
    {
        ...
        if (API_STOR.DecStat != HW_VOP_STARTED)
        {
            ...
            {
            case DEC_ERROR:
                ALOGE("MP4DecDecode: DEC_ERROR (line %d)", __LINE__);
                /*
            ...
            case DEC_ERROR_BUF_NOT_EMPTY:
                ALOGE("MP4DecDecode: DEC_ERROR_BUF_NOT_EMPTY (line %d)", __LINE__);
                /*
            ...
            case DEC_HDRS_RDY:
                ...
                if (pDecCont->ApiStorage.firstHeaders)
                {
                    ...
                    SetDecRegister(pDecCont->mp4Regs, HWIF_PIC_MB_H_EXT,
                                   pDecCont->VopDesc.vopHeight >> 8);
                ...
                {
                    /* Check mode validity */
                    if (DecCheckTiledMode(pDecCont->tiledModeSupport, 
                                    pDecCont->dpbMode,
            ...
            default:
                ...
                pDecCont->StrmStorage.status = STATE_SYNC_LOST;
                API_STOR.DecStat = INITIALIZED;
                ALOGE("MP4DecDecode: reached default, return MP4DEC_STRM_ERROR (line %d)", __LINE__);
                ret = MP4DEC_STRM_ERROR;
                break;
            }
        }
        ...
        if (API_STOR.DecStat == HW_VOP_STARTED)
        {
            if (DEC_VOPD.vopCoded)
            {
                ...
                if (asicStatus == X170_DEC_TIMEOUT)
                {
                    ALOGE("MP4DecDecode: MP4DEC_HW_TIMEOUT (line %d)", __LINE__);
                    ret = MP4DEC_HW_TIMEOUT;
                }
                else if (asicStatus == X170_DEC_SYSTEM_ERROR)
                {
                    ALOGE("MP4DecDecode: MP4DEC_SYSTEM_ERROR (line %d)", __LINE__);
                    ret = MP4DEC_SYSTEM_ERROR;
                }
                else if (asicStatus == X170_DEC_HW_RESERVED)
                {
                    ALOGE("MP4DecDecode: MP4DEC_HW_RESERVED (line %d)", __LINE__);
                    ret = MP4DEC_HW_RESERVED;
                }
                else if (asicStatus & MP4_DEC_X170_IRQ_BUS_ERROR)
                {
                    ALOGE("MP4DecDecode: MP4DEC_HW_BUS_ERROR (line %d)", __LINE__);
                    ret = MP4DEC_HW_BUS_ERROR;
                }
                        ...
                        asicStatus & MP4_DEC_X170_IRQ_TIMEOUT)
                {
                    ...
                    {
                        ALOGE("MP4DecDecode: MP4_DEC_X170_IRQ_STREAM_ERROR (line %d)", __LINE__);
...
#error Right bit-shifting (>>) does not preserve the sign
    ...
    *pDecInst = NULL; /* return NULL for any error */    
    ...
    {
        pDecCont->allowDpbFieldOrdering = config.fieldDpbSupport;
    }
...
#define DEC_REGS ((DecContainer *)decInst)->mp4Regs
    ...
    pDecInfo->dpbMode = ((DecContainer *)decInst)->dpbMode;   
            ...
            if (picIndex != BUFFER_UNDEFINED)
            {
                if (pPic[picIndex].sendToPp)
                {
                    ...
                    if (pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD && 
                        !pDecCont->ppConfigQuery.deinterlace)
                    {
                        pDecCont->ppControl.picStruct = (pDecCont->ppControl.inputBusLuma) ?
...
/*
    ...
    SetDecRegister(pDecContainer->mp4Regs, HWIF_DPB_ILACE_MODE, 
                    pDecContainer->dpbMode);                                   
...
{       
    ...
    {
        pDecCont->ppControl.picStruct = DECPP_PIC_TOP_AND_BOT_FIELD;
        pDecCont->ppControl.bottomBusLuma = pDecCont->ppControl.inputBusLuma +
            (pDecCont->VopDesc.vopWidth*pDecCont->VopDesc.vopHeight << 7);
        pDecCont->ppControl.bottomBusChroma = pDecCont->ppControl.inputBusChroma +
            (pDecCont->VopDesc.vopWidth*pDecCont->VopDesc.vopHeight << 6);
    }
    else
    {
        ASSERT(0);
...
static void MP4DecPrepareFieldProcessing(DecContainer * pDecCont, u32 picIndexOverride)
{
    ...
    if (pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD)
    {
        pDecCont->ppControl.picStruct = (pDecCont->VopDesc.topFieldFirst) ?
            DECPP_PIC_FRAME_OR_TOP_FIELD : DECPP_PIC_BOT_FIELD;
    }       
    ...
    else
    {
           ...
           && pDecCont->StrmStorage.pPicBuf[indexForPp].sendToPp)
        {
            ...
            if (pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD && 
                !pDecCont->ppConfigQuery.deinterlace)
            {
                pDecCont->ppControl.picStruct = (pDecCont->VopDesc.
                    topFieldFirst) ?
                    DECPP_PIC_FRAME_OR_TOP_FIELD : DECPP_PIC_BOT_FIELD;
            }    
