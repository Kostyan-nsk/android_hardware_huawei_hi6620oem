...
u32 StrmDec_DecodeShortVideoHeader(DecContainer * pDecContainer)
{
    ...
    if (pDecContainer->SvDesc.cpcf)
    {
        pDecContainer->SvDesc.temporalReference |=
           ((pDecContainer->SvDesc.etr & 0x3) << 8);
        tmp = pDecContainer->SvDesc.temporalReference;
        pDecContainer->VopDesc.ticsFromPrev = (tmp + 1024 -
                                                   (pDecContainer->SvDesc.
                                                    tics & 0x3FF)) & 0x3FF;
        pDecContainer->SvDesc.tics += pDecContainer->VopDesc.ticsFromPrev;
    }
...
static u32 StrmDec_DecodeSVHPlusHeader(DecContainer * pDecContainer)
{
    ...
    if (ufep == 1)
    {
        ...
        if (customSourceFormat)
        {
            ...
            pDecContainer->Hdrs.aspectRatioInfo = tmp;
        ...
        if (pDecContainer->SvDesc.cpcf)
            tmp = pDecContainer->SvDesc.cpcfc =
                StrmDec_GetBits(pDecContainer, 8);
    }

    /* extended temporal reference if custom picture clock frequency */
    if (pDecContainer->SvDesc.cpcf)
        tmp = pDecContainer->SvDesc.etr = StrmDec_GetBits(pDecContainer, 2);
