...
#define DEC_SVDS pDecCont->SvDesc
    ...
    if (DEC_STST.shortVideo)
    {
        ...
        if (DEC_SVDS.cpcf)
        {
            DEC_HDRS.vopTimeIncrementResolution = 1800000;
            if (DEC_SVDS.cpcfc >> 7) /* factor 1001 (5.1.7) */
                timeStep = (DEC_SVDS.cpcfc & 0x7F) * 1001;
            else                     /* factor 1000 (5.1.7) */
                timeStep = (DEC_SVDS.cpcfc & 0x7F) * 1000;
        }
        else
        {
        ...
        DEC_VOPD.vopTimeIncrement +=
            (pDecCont->VopDesc.ticsFromPrev * timeStep);
        while (DEC_VOPD.vopTimeIncrement >= DEC_HDRS.vopTimeIncrementResolution)
        {
            DEC_VOPD.vopTimeIncrement -= DEC_HDRS.vopTimeIncrementResolution;
