...
*/
    ...
    ALOGD("%s: offset = %lu, size = %d", __func__, base, dec_dwl->regSize);
    /* map the hw registers to user space */
//    dec_dwl->pRegBase = DWLMapRegisters(dec_dwl->fd_mem, base, dec_dwl->regSize, 1);
    dec_dwl->pRegBase = DWLMapRegisters(dec_dwl->fd, base, dec_dwl->regSize, 1);
...
i32 DWLRelease(const void *instance)
{
    ...
    switch (dec_dwl->clientType)
    {
        ...
        case DWL_CLIENT_TYPE_RV_DEC:
        case DWL_CLIENT_TYPE_AVS_DEC:
            if (hw_performance_log) {
                DWL_DEBUG("dec;%d;%d", dec_pic_counter, dec_time);
                fprintf(hw_performance_log, "dec;%d;%d\n", dec_pic_counter, dec_time);
            }
        break;
        case DWL_CLIENT_TYPE_PP:
            if (hw_performance_log) {
                DWL_DEBUG("pp;%d;%d\n", pp_pic_counter, pp_time);
                fprintf(hw_performance_log, "pp;%d;%d\n", pp_pic_counter, pp_time);
            }
    }
#endif
...
i32 DWLWaitDecHwReady(const void *instance, u32 timeout)
{
    ...
    {
        ret = ioctl(dec_dwl->fd, HX170DEC_IOC_WAITFORDEC, NULL);
	DWL_DEBUG("HX170DEC_IOC_WAITFORDEC: ret = %d cnt = %d", ret, cnt);
    }
...
i32 DWLWaitPpHwReady(const void *instance, u32 timeout)
{
    ...
    while (!DWL_PP_INT)
    {
	ret = ioctl(dec_dwl->fd, HX170DEC_IOC_WAITFORPP, NULL);
	DWL_DEBUG("HX170DEC_IOC_WAITFORPP: ret = %d cnt = %d", ret, cnt);
    }
