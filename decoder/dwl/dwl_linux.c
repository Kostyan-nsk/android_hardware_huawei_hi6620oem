                     ...
                     unsigned int regSize, u32 write)
{
    ...
    if (write)
        io = mmap64(0, mapSize, PROT_READ | PROT_WRITE,
                           MAP_SHARED, mem_dev, base & ~pageAlignment);
    else
        io = mmap64(0, mapSize, PROT_READ,
                           MAP_SHARED, mem_dev, base & ~pageAlignment);
...
i32 DWLMallocRefFrm(const void *instance, u32 size, DWLLinearMem_t * info)
{
    ...
    DWL_DEBUG("DWLMallocRefFrm\t%8d bytes\n", size);
#endif
...
i32 DWLMallocLinear(const void *instance, u32 size, DWLLinearMem_t * info)
{
    ...
    info->virtualAddress = (u32 *) mmap64(0, info->size, PROT_READ | PROT_WRITE,
                                        MAP_SHARED, dec_dwl->fd,
                                        params.busAddress);

#ifdef MEMORY_USAGE_TRACE
    DWL_DEBUG("DWLMallocLinear 0x%08x virtualAddress: 0x%08x\n",
	...
	DWL_DEBUG("zero_copy: info->virtualAddress = 0x%p", info->virtualAddress);
	info->size -= 64;
	info->busAddress += 64;
