...
#ifdef ANDROID_LOG
#define LOG_TAG "G1_PP"
#include <log/log.h>
#define LOG_ANDROID(args...)  ALOGD(args)
#else
#define LOG_ANDROID(args...) do{}while (0)
#endif
...
#error "Bad value specified for PP_X170_BUS_BURST_LENGTH"
    ...
    ppC->jpeg16kSupport = hwCfg.webpSupport; /* flag reused for JPEG as
                                              * well */
...
void PPSetupHW(PPContainer * ppC)
{
    ...
    LOG_ANDROID("%s: ppOutDeint->enable = %d", __func__, ppOutDeint->enable);
    ...
    if (ppC->decInst == NULL)
    {
        ...
        LOG_ANDROID("%s: ppInImg->picStruct = %u", __func__, ppInImg->picStruct);
...
i32 PPCheckTiledOutput(PPContainer *ppC, PPConfig * ppCfg)
{
    ...
    if (ppOutImg->width & mask) {
        LOG_ANDROID("PPCheckTiledOutput: PP_SET_OUT_SIZE_INVALID (%u)", __LINE__);
    ...
    if (ppOutMask1->enable)
    {
        ...
        {
            LOG_ANDROID("PPCheckTiledOutput: PP_SET_MASK1_INVALID (%u)", __LINE__);
    ...
    if (ppOutMask2->enable)
    {
        ...
        {
            LOG_ANDROID("PPCheckTiledOutput: PP_SET_MASK2_INVALID (%u)", __LINE__);
    ...
    {
        LOG_ANDROID("PPCheckTiledOutput: PP_SET_FRAMEBUFFER_INVALID (%u)", __LINE__);
                  ...
                  u32 decLinked, u32 decType)
{
    ...
    {
        LOG_ANDROID("%s: PP_SET_IN_FORMAT_INVALID (%u)", __func__, __LINE__);
    ...
    if (!decLinked)
    {
           ...
           ppInImg->picStruct != PP_PIC_BOT_FIELD_FRAME)
        {
            ...
            {
                LOG_ANDROID("%s: PP_SET_IN_ADDRESS_INVALID (%u)", __func__, __LINE__);
    ...
    if (ppC->hwId == 0x8170U)
    {
        ...
        {
            LOG_ANDROID("%s: PP_SET_IN_SIZE_INVALID (%u)", __func__, __LINE__);
    ...
    else
    {
        ...
        u32 maxMbs = ((decType == PP_PIPELINED_DEC_TYPE_WEBP) ||
    ...
    {
        LOG_ANDROID("%s: PP_SET_IN_STRUCT_INVALID (%u)", __func__, __LINE__);
    ...
    if (ppInCrop->enable != 0)
    {
        ...
        {
            LOG_ANDROID("%s: PP_SET_CROP_INVALID (%u)", __func__, __LINE__);
    ...
    switch (ppInRotation->rotation) {
        ...
        case PP_ROTATION_180:
            break;
        default:
            LOG_ANDROID("%s: PP_SET_ROTATION_INVALID (%u)", __func__, __LINE__);
    ...
    {
        LOG_ANDROID("%s: PP_SET_OUT_FORMAT_INVALID (%u)", __func__, __LINE__);
        return (i32) PP_SET_OUT_FORMAT_INVALID;
    }

    if (ppOutImg->bufferBusAddr == 0 || ppOutImg->bufferBusAddr & address_mask)
    {
        LOG_ANDROID("%s: PP_SET_OUT_ADDRESS_INVALID (%u)", __func__, __LINE__);
    ...
    {
        LOG_ANDROID("%s: PP_SET_OUT_SIZE_INVALID (%u)", __func__, __LINE__);
    ...
    /* scale check */
    {
        ...
        if (!ppC->scalingEna)
        {
            if ((w != ppOutImg->width) || (h != ppOutImg->height))
                LOG_ANDROID("%s: PP_SET_SCALING_UNSUPPORTED (%u)", __func__, __LINE__);
    ...
    if (ppOutFrmBuffer->enable)
    {
        ...
        {
            LOG_ANDROID("%s: PP_SET_FRAMEBUFFER_INVALID (%u)", __func__, __LINE__);
    ...
    if ((ppOutImg->pixFormat & PP_PIXEL_FORMAT_RGB_MASK))
    {
        ...
        {
            LOG_ANDROID("%s: PP_SET_DITHERING_UNSUPPORTED (%u)", __func__, __LINE__);
        ...
        {
            LOG_ANDROID("%s: PP_SET_VIDEO_ADJUST_INVALID (%u)", __func__, __LINE__);
        ...
        if (ppOutImg->pixFormat == PP_PIX_FMT_RGB32_CUSTOM)
        {
            ...
            if ((rgbbm->maskR & rgbbm->maskG & rgbbm->maskB & rgbbm->
                maskAlpha) != 0)
            {
                LOG_ANDROID("%s: PP_SET_RGB_BITMASK_INVALID (%u)", __func__, __LINE__);
    ...
    if (ppOutMask1->enable && ppOutMask1->alphaBlendEna)
    {
        ...
        {
            LOG_ANDROID("%s: PP_SET_MASK1_INVALID (%u)", __func__, __LINE__);
    ...
    if (ppOutMask2->enable && ppOutMask2->alphaBlendEna)
    {
        ...
        {
            LOG_ANDROID("%s: PP_SET_MASK2_INVALID (%u)", __func__, __LINE__);
    ...
    if (ppOutDeint->enable)
    {
        if (!ppC->deintEna)
        {
            LOG_ANDROID("%s: PP_SET_DEINTERLACING_UNSUPPORTED (%u)", __func__, __LINE__);
        ...
        if (ppInImg->pixFormat != PP_PIX_FMT_YCBCR_4_2_0_SEMIPLANAR &&
           ...
           ppInImg->pixFormat != PP_PIX_FMT_YCBCR_4_0_0)
        {
            LOG_ANDROID("%s: PP_SET_DEINTERLACE_INVALID (%u)", __func__, __LINE__);
    ...
    {
        LOG_ANDROID("%s: PP_SET_IN_RANGE_MAP_INVALID (%u)", __func__, __LINE__);
...
u32 PPIsInPixFmtOk(u32 pix_fmt, const PPContainer * ppC)
{
    ...
    LOG_ANDROID("%s: pix_fmt = 0x%x", __func__, pix_fmt);
    switch (pix_fmt)
    {
                                 ...
                                 u32 ablendCrop)
{
    ...
    {
        LOG_ANDROID("%s: PP_SET_ABLEND_UNSUPPORTED (%u)", __func__, __LINE__);
...
u32 PPCheckSetupChanges(PPConfig * prevCfg, PPConfig * newCfg)
{
    ...
    /* checked because changes pipeline status */
