...
/* Code */
    ...
    if (pDecCont->ppInstance != NULL)
    {
        ...
        if (pDecCont->storage.interlace)
            pDecCont->ppConfigQuery.pipelineAccepted = 0;
    }
    else
    {
    ...
    else
    {
        ...
        * decode second field and wait PP after that */
        ...
        switch(pPic[picIndex].tiledMode) 
        {
            case 0: pPicture->outputFormat = DEC_OUT_FRM_RASTER_SCAN; break;
            case 1: pPicture->outputFormat = DEC_OUT_FRM_TILED_8X4; break;
        }
        ...
        if (pDecCont->ppInstance != NULL)
        {
            ...
            if (pPic[picIndex].field[fieldToReturn].decPpStat == STAND_ALONE)
            {
                ...
                {
                    pDecCont->ppControl.bottomBusLuma = pPicture->outputPictureBusAddress +
