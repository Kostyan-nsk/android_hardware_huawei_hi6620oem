...
#define H264DEC_BUILD_MINOR 275
...
#ifdef ANDROID_LOG
#define LOG_ANDROID(...)	ALOGD(__VA_ARGS__)
#else
#define LOG_ANDROID(...)	do{}while (0)
#endif
...
#else
#undef TRACE_PP_CTRL
#define TRACE_PP_CTRL(...)	ALOGD(__VA_ARGS__)
#endif

#ifdef H264DEC_TRACE
#define DEC_API_TRC(...)	ALOGD(__VA_ARGS__)
#else
#define DEC_API_TRC(...)	do{}while (0)
#endif
...
#ifdef ANDROID_LOG
    LOG_ANDROID("HWIF_DEC_OUT_ENDIAN = %d", DEC_X170_OUTPUT_PICTURE_ENDIAN);
    LOG_ANDROID("HWIF_DEC_IN_ENDIAN = %d", DEC_X170_INPUT_DATA_ENDIAN);
    LOG_ANDROID("HWIF_DEC_STRENDIAN_E = %d", DEC_X170_INPUT_STREAM_ENDIAN);
    LOG_ANDROID("HWIF_DEC_MAX_BURST = %d", DEC_X170_BUS_BURST_LENGTH);
    LOG_ANDROID("HWIF_DEC_OUTSWAP32_E = %d", DEC_X170_OUTPUT_SWAP_32_ENABLE);
    LOG_ANDROID("HWIF_DEC_INSWAP32_E = %d", DEC_X170_INPUT_DATA_SWAP_32_ENABLE);
    LOG_ANDROID("HWIF_DEC_STRSWAP32_E = %d", DEC_X170_INPUT_STREAM_SWAP_32_ENABLE);
#endif
        ...
        case H264BSD_PIC_RDY:
            {
                ...
                if (pDecCont->rlcMode)
                {
                    ...
                    {
                        DEBUG_PRINT("H264DecDecode# IRQ_STREAM_ERROR in RLC mode)!\n");
                    }
                ...
                else if (asic_status & DEC_X170_IRQ_BUFFER_EMPTY)
                {
                    ...
                    if (pDecCont->forceNalMode)
                    {
                        ...
                        next = 
                            h264bsdFindNextStartCode(pDecCont->pHwStreamStart-1,
                                                     pDecCont->hwLength+1);
    ...
    if (pStorage->validSliceInAccessUnit)
    {
                                      ...
                                      !pStorage->baseOppositeFieldPic))
            {
                ...
                for (i = 0; i < pStorage->dpb->numOut; i++, tmp++)
                {
                       ...
                       pStorage->dpb->outBuf[tmp].data)
                    {
                        ...
                        DEBUG_PRINT("h264UpdateAfterPictureDecode: Current frame in output list; ");
        ...
        else
        {
            if (!pStorage->dpb->noReordering)
                h264DpbUpdateOutputList(pStorage->dpb, pStorage->currImage);
            DEBUG_PRINT("h264UpdateAfterPictureDecode: Output all delayed pictures!\n");
