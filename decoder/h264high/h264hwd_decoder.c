...
/*------------------------------------------------------------------------------
    ...
    Function: h264bsdFixFrameNum
        ...
        Functional description:
            ...
            implements SW workaround for hardware fault (bit 12 of frameNum
...
#define BIT 12
    ...
    *skippedBytes = 0;
    ...
    while (maxFrameNum >> frameNumLen)
        frameNumLen++;
    frameNumLen--;

    ASSERT(frameNumLen > BIT);
    ...
    while (1)
    {
            ...
            tmp == NAL_CODED_SLICE_EXT)
        {
            ...
            strmData.pStrmBuffStart = p;
            strmData.pStrmCurrPos = strmData.pStrmBuffStart;
            strmData.bitPosInWord = 0;
            strmData.strmBuffReadBits = 0;
            strmData.strmBuffSize = pEnd - p;

            /* first_mb_in_slice */
            tmp = h264bsdDecodeExpGolombUnsigned(&strmData, &value);
            if (tmp != HANTRO_OK)
                return 0;
            ...
            /* frame_num */
            tmp = h264bsdShowBits(&strmData, frameNumLen);
            if (tmp != frameNum)
                return 0;

            bitPos = strmData.bitPosInWord + frameNumLen - 1 - BIT;
            p = (u8*)strmData.pStrmCurrPos + bitPos/8;
            bitPos &= 0x7;
            /* reset bit */
            *p &= ~(0x80 >> bitPos);
        ...
        if (loop)
        {
            p = (u8*)h264bsdFindNextStartCode(p, pEnd - p);
            if (p == NULL)
                return 0;
        }
        else
