...
//#define LOG_NDEBUG 0
#endif
#define TRACE_PP_CTRL(args...)          ALOGV(args)
#endif
...
void h264PreparePpRun(decContainer_t * pDecCont)
{
    ...
    if (dpb->noReordering && (!dpb->numOut || dpb->delayedOut))
    {
        TRACE_PP_CTRL
    ...
    {
        decPpIf->ppStatus = DECPP_RUNNING;
        TRACE_PP_CTRL("h264PreparePpRun: pipeline=ON => PP will be running\n");
    }
    else
    {
        if (pDecCont->pp.queuedPicToPp[multiView] != NULL)
        {
            decPpIf->ppStatus = DECPP_RUNNING;
            TRACE_PP_CTRL("h264PreparePpRun: pipeline=OFF, queued picture => PP run in parallel\n");
        }
        else
        {
            TRACE_PP_CTRL("h264PreparePpRun: pipeline=OFF, no queued picture => PP no run\n");
