...
//#define LOG_NDEBUG 0
#define LOG_TAG "vp8hwd_asic"
#include <cutils/log.h>
#define TRACE_PP_CTRL(str...) ALOGV(str)
...
/*        
    ...
    if (pDecCont->hwEcSupport)
    {
        ...
        dwl_ret = DWLMallocLinear(dwl, memorySize, &pAsicBuff->mvs[0]);
        if (dwl_ret != DWL_OK)
        {
            VP8HwdAsicReleasePictures(pDecCont);
            return -1;
        }
        dwl_ret = DWLMallocLinear(dwl, memorySize, &pAsicBuff->mvs[1]);
        if (dwl_ret != DWL_OK)
        {
            VP8HwdAsicReleasePictures(pDecCont);
            return -1;
...
void VP8HwdAsicInitPicture(VP8DecContainer_t * pDecCont)
{
    ...
    {
        SetDecRegister(pDecCont->vp8Regs, HWIF_REFER2_BASE,
                      pAsicBuff->picturesC[pAsicBuff->goldenBufferI].busAddress);
    }                       
    ...
    {
        SetDecRegister(pDecCont->vp8Regs, HWIF_REFER3_BASE,
               pAsicBuff->picturesC[pAsicBuff->alternateBufferI].busAddress);
    }                       
...
u32 VP8HwdAsicRun(VP8DecContainer_t * pDecCont)
{
    ...
    DEBUG_PRINT("[%d] after GetDecRegister(): asic_status = 0x%x", __LINE__, asic_status);
