...
static u32 ReadPartitionSize(const u8 *cxSize);

static u32 DecodeVp8FrameHeader(const u8 *pStrm, u32 strmLen, vpBoolCoder_t*bc, 
                                  vp8Decoder_t* dec);
...
------------------------------------------------------------------------------*/
u32 DecodeVp8FrameHeader(const u8 *pStrm, u32 strmLen, vpBoolCoder_t*bc, 
                          vp8Decoder_t* dec)
{
    u32 tmp;
    u32 i;
...
u32 vp8hwdSetPartitionOffsets(const u8 *stream, u32 len, vp8Decoder_t *dec)
{
    ...
    for (i = 0 ; i < dec->nbrDctPartitions - 1 ; ++i)
    {
        ...
        if (dec->dctPartitionOffsets[i] < len)
        {
            offset += ReadPartitionSize(stream);
        }
        else
        {
            dec->dctPartitionOffsets[i] = len - 1;
        }
        stream += 3;
    }
    dec->dctPartitionOffsets[i] = baseOffset + offset;
...
------------------------------------------------------------------------------*/
u32 ReadPartitionSize(const u8 *cxSize)
{
	u32 size; 
	size =	(u32)(*cxSize)
		 +	((u32)(* (cxSize+1))<<8) 
