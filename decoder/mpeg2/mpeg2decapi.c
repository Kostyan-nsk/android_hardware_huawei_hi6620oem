...
#define MPEG2DEC_BUILD_MINOR 239
            ...
            case DEC_HDRS_RDY:
                ...
                if (pDecCont->ApiStorage.firstHeaders)
                {
                    ...
                    if ((!pDecCont->Hdrs.progressiveSequence ||
                         !pDecCont->Hdrs.framePredFrameDct) && 
                ...
                !pDecCont->StrmStorage.parallelMode2)
            {
                ...
                if (MPEG2DEC_IS_FIELD_OUTPUT)
                {
                       ...
                       DECPP_PIC_BOT_FIELD_FRAME)
                    {
                        ...
                        {
                            u32 frameSize = (pDecCont->FrameDesc.frameWidth *
                                                    pDecCont->FrameDesc.
                                                    frameHeight) << 8;
                            pDecCont->ppControl.bottomBusLuma =
                                pDecCont->StrmStorage.pPicBuf[picIndex].
                                data.busAddress + frameSize/2;

                            pDecCont->ppControl.bottomBusChroma =
                                pDecCont->StrmStorage.pPicBuf[picIndex].
                                data.busAddress + frameSize + frameSize/4;
                        }
                    }
                }
            }
...
static u32 Mpeg2SetRegs(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    */
    ...
    SetDecRegister(pDecContainer->mpeg2Regs, HWIF_PIC_MB_H_EXT,
    ...
    {    
        ...
        if (pDecContainer->dpbMode == DEC_DPB_DEFAULT)
        {
            SetDecRegister(pDecContainer->mpeg2Regs, HWIF_DEC_OUT_BASE,
                           (pDecContainer->StrmStorage.pPicBuf[pDecContainer->
                                                               StrmStorage.workOut].
        ...
        else if (pDecContainer->dpbMode == DEC_DPB_INTERLACED_FIELD)
        {
            SetDecRegister(pDecContainer->mpeg2Regs, HWIF_DEC_OUT_BASE,
                           pDecContainer->StrmStorage.pPicBuf[pDecContainer->
                                                              StrmStorage.workOut].
    ...
    SetDecRegister(pDecContainer->mpeg2Regs, HWIF_DPB_ILACE_MODE, 
...
static void Mpeg2DecSetupDeinterlace(DecContainer * pDecCont)
{
    ...
    else if (pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD)
    {
        ...
        pDecCont->ppControl.bottomBusLuma = pDecCont->ppControl.inputBusLuma +
            (pDecCont->FrameDesc.frameWidth*pDecCont->FrameDesc.frameHeight << 7);
        pDecCont->ppControl.bottomBusChroma = pDecCont->ppControl.inputBusChroma +
            (pDecCont->FrameDesc.frameWidth*pDecCont->FrameDesc.frameHeight << 6);
    }
    else
    {
        ASSERT(0);
    ...
    u32 picIndexOverride)
{
    ...
    if (pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD)
    {
        pDecCont->ppControl.picStruct = (pDecCont->Hdrs.topFieldFirst) ?
            DECPP_PIC_FRAME_OR_TOP_FIELD : DECPP_PIC_BOT_FIELD;
    }
    ...
    else
    {
           ...
           && pDecContainer->StrmStorage.pPicBuf[indexForPp].sendToPp)
        {
            ...
            if (pc->picStruct == DECPP_PIC_FRAME_OR_TOP_FIELD ||
               ...
               pc->picStruct == DECPP_PIC_TOP_AND_BOT_FIELD)
            {
