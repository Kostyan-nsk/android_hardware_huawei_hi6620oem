...
OMXPluginBase *createOMXPlugin() {
        ...
        mGetRolesOfComponentHandle (NULL)
    {
        if (mLibHandle != NULL) {
            ...
            mComponentNameEnum = (ComponentNameEnumFunc) dlsym (mLibHandle, "hantro_OMX_ComponentNameEnum");
            ...
            (*mInit) ();
            ALOGV("Sucessfully allocated and initialized OMX plugin");
        }
    }

    OMXPlugin::~OMXPlugin () {
        ALOGV("%s start mDeinit = %p", __func__, mDeinit);
        if (mLibHandle != NULL && mDeinit != NULL) {
            ALOGV("%s: call hantro_OMX_DeInit", __func__);
            (*mDeinit)();
        ...
        OMX_COMPONENTTYPE **component)
    {
        ...
        return (*mGetHandle) (reinterpret_cast <
            OMX_HANDLETYPE * >(component),
            const_cast < char *>(name), appData,
            const_cast < OMX_CALLBACKTYPE * >(callbacks));
    }

    OMX_ERRORTYPE OMXPlugin::destroyComponentInstance (OMX_COMPONENTTYPE *component)
    {
        if (mLibHandle == NULL) {
            return OMX_ErrorUndefined;
        }
    ...
    OMX_ERRORTYPE OMXPlugin::enumerateComponents (OMX_STRING name, size_t size, OMX_U32 index)
    {
        if (mLibHandle == NULL) {
            return OMX_ErrorUndefined;
        }
    ...
    OMX_ERRORTYPE OMXPlugin::getRolesOfComponent (const char *name, Vector < String8 > *roles)
    {
        roles->clear ();
