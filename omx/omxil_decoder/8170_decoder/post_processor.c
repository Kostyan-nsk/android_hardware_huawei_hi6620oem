                                           ...
                                           OMX_BOOL bDeIntEna)
{
		...
		args->format != OMX_COLOR_FormatYUV420PackedSemiPlanar))
    {
        ...
        pp_config->ppOutImg.pixFormat = format;
...
//            pp_config->ppOutRgb.rgbTransform = PP_YCBCR2RGB_TRANSFORM_BT_601;
            /* Set BT.601 for SD and BT.709 for HD */
            if ((pp_config->ppOutImg.width < 1280 && pp_config->ppOutImg.height < 720) ||
                (pp_config->ppOutImg.width < 720 && pp_config->ppOutImg.height < 1280))
            {
                pp_config->ppOutRgb.rgbTransform = PP_YCBCR2RGB_TRANSFORM_BT_601;
            }
            else
            {
                pp_config->ppOutRgb.rgbTransform = PP_YCBCR2RGB_TRANSFORM_BT_709;
            }
