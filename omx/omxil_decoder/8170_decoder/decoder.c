...
#ifdef DECODER_COLOR_FORMAT_NV21
#include <arm_neon.h>
#endif
...
#define MAX_TICK_COUNTS 16

#ifdef OMX_SKIP64BIT
#define EARLIER(t1, t2) (((t1).nHighPart <= (t2).nHighPart) && ((t1).nLowPart < (t2).nLowPart))
#else
...
static void receive_timestamp(OMX_DECODER *dec, OMX_TICKS *timestamp)
{
    ...
    {
        OMX_U32 capacity = temp->capacity + 8;
...
static OMX_U32 decoder_thread_main(BASECOMP * base, OMX_PTR arg)
{
    ...
    while (this->run)
    {
        ...
        {
            CMD cmd;
	    TRACE_PRINT("ASYNC: received signal from command queue");
            while (1)
            {
...
static OMX_ERRORTYPE decoder_deinit(OMX_IN OMX_HANDLETYPE hComponent)
{
    ...
    if (dec->state != OMX_StateLoaded)
    {
        ...
        TRACE_PRINT("API: dealloc buffers done\n");
        if (dec->codec)
                                       ...
                                       OMX_IN OMX_PTR pCmdData)
{
    ...
    case OMX_CommandStateSet:
        if (dec->statetrans != dec->state)
        {
            ...
            ALOGE("%s: OMX_CommandStateSet: %d != %d (transition already pending)", __func__, dec->statetrans, dec->state);
    ...
    TRACE_PRINT("API: %s end", __func__);
    return err;
}
                                     ...
                                     OMX_IN OMX_U8 * pBuffer)
{
    ...
    if (dec->useNativeBuf && nPortIndex == PORT_INDEX_OUTPUT) {
	struct private_handle_t *handle = (struct private_handle_t *)pBuffer;
	buff->header->nAllocLen = handle->size;
	buff->bus_address = handle->phys_addr;
	buff->bus_data = handle->base;
	buff->flags |= BUFFER_FLAG_NATIVE_BUFFER;
	ALOGV("API: using native buffer: size = %d width = %d height = %d phy = 0x%x", handle->size, handle->width, handle->height, handle->phys_addr);
    }
    else {
                                          ...
                                          OMX_IN OMX_U32 nSizeBytes)
{
    ...
    {
        TRACE_PRINT("API: port is populated\n");
        TRACE_PRINT("API: bufferqueue.size = %u\n",port->bufferqueue.size);
        port->def.bPopulated = OMX_TRUE;
    }
    *ppBuffer = buff->header;
                                        ...
                                        OMX_IN OMX_PTR pParam)
{
    ...
    CHECK_STATE_INVALID(dec->state);
    TRACE_PRINT("API: %s\n", __FUNCTION__);
    TRACE_PRINT("API: setting index: %s\n",
    ...
    case OMX_IndexParamPortDefinition:
		...
		TRACE_PRINT("API: OMX_IndexParamPortDefinition PORT_INDEX_INPUT nBufferCountActual = %d nBufferSize = %d eColorFormat = %d nFrameWidth = %d nFrameHeight = %d",
			    param->nBufferCountActual, param->nBufferSize, param->format.video.eColorFormat, param->format.video.nFrameWidth, param->format.video.nFrameHeight);
                port->nBufferCountActual = param->nBufferCountActual;
//                port->nBufferSize = param->nBufferSize;
                port->nBufferSize = param->format.video.nFrameWidth * param->format.video.nFrameHeight * 2;
                ...
                dec->out.def.nBufferSize =
                    ((param->format.video.nFrameWidth + 1) & ~1) * param->format.video.nFrameHeight * 4;
#else
                dec->out.def.nBufferSize = (ALIGN(param->format.video.nFrameWidth, 16) * ALIGN(param->format.video.nFrameHeight, 16) * 3) / 2;
#endif
            ...
            case PORT_INDEX_OUTPUT:
                port = &dec->out.def;
                CHECK_PORT_STATE(dec, port);
		TRACE_PRINT("API: OMX_IndexParamPortDefinition PORT_INDEX_OUTPUT nBufferCountActual = %d nBufferSize = %d eColorFormat = %d nFrameWidth = %d nFrameHeight = %d",
			    param->nBufferCountActual, param->nBufferSize, param->format.video.eColorFormat, param->format.video.nFrameWidth, param->format.video.nFrameHeight);
                port->nBufferCountActual = param->nBufferCountActual;
                port->nBufferSize = param->nBufferSize;
                port->format.video.eColorFormat =
	...
	case OMX_GoogleIndexEnableAndroidNativeBuffers:
	{
		...
		if (param->nPortIndex != PORT_INDEX_OUTPUT)
		{
			ALOGE("%s: Bad port index %d", __func__, (int)param->nPortIndex);
			return OMX_ErrorBadPortIndex;
		}
		dec->useNativeBuf = param->enable;
		ALOGD("useNativeBuf = %u", dec->useNativeBuf);
		break;
	}
#endif
    default:
        TRACE_PRINT("API: unsupported settings index 0x%x", nIndex);
    ...
    case OMX_IndexParamPortDefinition:
        {
            ...
            OMX_PARAM_PORTDEFINITIONTYPE *param =
                (OMX_PARAM_PORTDEFINITIONTYPE *) pParam;
	    TRACE_PRINT("API: nPortIndex = %d", param->nPortIndex);
            switch (param->nPortIndex)
            {
            case PORT_INDEX_INPUT:
                port = &dec->in.def;
...
// this is for the post-processor input
   ...
   case OMX_IndexParamVideoRv:
    ...
    {
	struct GetAndroidNativeBufferUsageParams *pUsage = (struct GetAndroidNativeBufferUsageParams *)pParam;

	if ((pUsage->nPortIndex != PORT_INDEX_OUTPUT) || !dec->useNativeBuf)
	{
		ALOGE("Bad conditions: nPortIndex=%u, useNativeBuf=%d\n", pUsage->nPortIndex, dec->useNativeBuf);
		return OMX_ErrorUndefined;
	}
	else
		pUsage->nUsage = GRALLOC_USAGE_HW_VIDEO_ENCODER;
    ...
    case OMX_IndexParamPortDefinition:
        {
            ...
            OMX_PARAM_PORTDEFINITIONTYPE *param =
                (OMX_PARAM_PORTDEFINITIONTYPE *) pParam;
	    TRACE_PRINT("OMX_IndexParamPortDefinition port = %d", param->nPortIndex);
            switch (param->nPortIndex)
            {
            case PORT_INDEX_INPUT:
                port = &dec->in.def;
                                            ...
                                            pBufferHeader)
{
	...
	if (pBufferHeader->nTimeStamp == 0)
	    ALOGE("input buffer timestamp = 0!");
    }
#endif
        ...
        OMX_OUT OMX_INDEXTYPE* pIndexType)
{
	...
	{
		*pIndexType = (OMX_INDEXTYPE)OMX_GoogleIndexEnableAndroidNativeBuffers;
	}
	else if (!strcmp(cParameterName, "OMX.google.android.index.getAndroidNativeBufferUsage"))
	{
		*pIndexType = (OMX_INDEXTYPE)OMX_GoogleIndexGetAndroidNativeBufferUsage;
	}
	else if (!strcmp(cParameterName, "OMX.google.android.index.useAndroidNativeBuffer2"))
	{
		*pIndexType = (OMX_INDEXTYPE)OMX_GoogleIndexUseAndroidNativeBuffer2;
	}
	else
	{
...
/*    tbCfg.decParams.h264Support = 1;
...
#else
    err = HantroOmx_port_init(&dec->in, 4, 8, 8, 4194304);
#endif
    if (err != OMX_ErrorNone)
        goto INIT_FAILURE;
    ...
    err = HantroOmx_port_init(&dec->out, 4, 8, 8, 4194304);
#endif
#endif

#ifdef OMX_DECODER_IMAGE_DOMAIN
    err = HantroOmx_port_init(&dec->out, 4, 4, 5, 3686400);
#endif
    if (err != OMX_ErrorNone)
        goto INIT_FAILURE;
...
//    dec->out.def.format.video.eColorFormat = OMX_COLOR_FormatYUV420Planar;
    dec->out.def.format.video.eColorFormat = OMX_COLOR_FormatYUV420PackedSemiPlanar;
...
static OMX_ERRORTYPE transition_to_idle_from_loaded(OMX_DECODER * dec)
{
    ...
    case OMX_VIDEO_CodingUnused:
        ...
        case OMX_COLOR_FormatYCbYCr:   // 422-interleaved
            ...
            TRACE_PRINT("ASYNC: raw frame width: %d height: %d color format: %s\n",
...
#endif // OMX_DECODER_IMAGE_DOMAIN
    if (!dec->codec) {
        ALOGE("ASYNC: no codec was created");
        goto FAIL;
    }
    ...
    if (err != OMX_ErrorNone) {
        ALOGE("ASYNC: error allocating temporary input frame buffer");
        goto FAIL;
    }
    ...
    err = OSAL_AllocatorAllocMem(&dec->alloc, &output_buffer_size,
                                &dec->frame_out.bus_data,
                                &dec->frame_out.bus_address);
    if (err != OMX_ErrorNone) {
        ALOGE("ASYNC: error allocating temporary output frame buffer");
        goto FAIL;
    }
    ...
    if (dec->inpp.def.bEnabled)
    {
        if (mask_buffer_size > 0)
        {
            ...
            if (err != OMX_ErrorNone) {
                ALOGE("ASYNC: error allocating mask input frame buffer (1)");
                goto FAIL;
            }
        }
/*
        else
        {
            ...
            err =
                OSAL_AllocatorAllocMem(&dec->alloc, &mask_buffer_size,
                                       &dec->mask.bus_data, &dec->mask.bus_address);
            if (err != OMX_ErrorNone) {
                ALOGE("ASYNC: error allocating mask input frame buffer (2)");
                goto FAIL;
            }
        }
*/
    ...
    TRACE_PRINT("-------------> set post-processor scaler: %dx%d", args.scale.width, args.scale.height);
...
static OMX_ERRORTYPE async_set_new_pp_args(OMX_DECODER *dec, OMX_U32 newWidth, OMX_U32 newHeight)
{
	...
	if (newWidth < 64 ||
		...
		((newWidth < dec->originalWidth) && (newHeight > dec->originalHeight)))
	{
		ALOGE("async_set_new_pp_args: wrong dimensions (%dx%d) original dimensions (%dx%d)", newWidth, newHeight, dec->originalWidth, dec->originalHeight);
		return OMX_ErrorBadParameter;
	}
...
static OMX_ERRORTYPE async_decoder_return_buffers(OMX_DECODER * dec, PORT * p)
{
    ...
    TRACE_PRINT("-------> [TIMESTAMP_FIX] async_decoder_return_buffers (%d) [head = %u, tail = %u]", p->def.eDir, dec->timestampsQueueHead, dec->timestampsQueueTail);
    if (p->def.eDir == OMX_DirInput)
    {
        dec->timestampsQueueHead = dec->timestampsQueueTail;
        TRACE_PRINT("-------> [TIMESTAMP_FIX] tail = head = %u", dec->timestampsQueueHead);
    }
#endif
...
#ifdef ANDROID
static void power_module_hint(OMX_BOOL executing, OMX_U32 frameWidth, OMX_U32 frameHeight)
{
    power_module_t *power_module;
    struct powerData power_data;
    ...
    if (power_module->powerHint) {
        power_data.executing   = executing;
        power_data.frameWidth  = frameWidth;
        power_data.frameHeight = frameHeight;
        power_module->powerHint(power_module, POWER_HINT_VIDEO_DECODE, (void *)&power_data);
    }
}
#endif
...
#ifdef ANDROID
    power_module_hint(OMX_FALSE, dec->out.def.format.video.nFrameWidth,
...
static OMX_ERRORTYPE transition_to_loaded_from_idle(OMX_DECODER * dec)
{
    ...
    {
	TRACE_PRINT("ASYNC: waiting for buffers release...");
        OSAL_ThreadSleep(RETRY_INTERVAL);
    }
...
#ifdef ANDROID
    power_module_hint(OMX_TRUE, dec->out.def.format.video.nFrameWidth,
                                dec->out.def.format.video.nFrameHeight);
#endif
...
static void async_decoder_wait_port_buffer_dealloc(OMX_DECODER * dec, PORT * p)
{
    ...
    {
		TRACE_PRINT("%s: waiting for port buffers dealloc: %u",__func__, p->def.nPortIndex);
        OSAL_ThreadSleep(RETRY_INTERVAL);
    }
	TRACE_PRINT("ASYNC: %s done\n", __FUNCTION__);
}
...
static OMX_ERRORTYPE async_get_info(OMX_DECODER * dec)
{
    ...
    TRACE_PRINT("--> dec->codec->getinfo returned video size of %ux%u",info.width, info.height);
...
//        TRACE_PRINT("ASYNC: Video Frame Width/Height Changed.");
	ALOGW("ASYNC: Video Frame Size Changed");
    }
    if (portResChanged)
#endif
    {
...
static OMX_ERRORTYPE async_get_frame_buffer(OMX_DECODER * dec, FRAME * frm)
{
    ...
    TRACE_PRINT("%s: dec->state = %d dec->statetrans = %d dec->outputPortFlushPending = %d",
		__func__, dec->state, dec->statetrans, dec->outputPortFlushPending);
    int retry = MAX_RETRIES; // needed for stagefright when video is paused (no transition to pause)
    while (buff == NULL && retry > 0 && dec->state != OMX_StatePause && dec->statetrans != OMX_StatePause &&
            dec->statetrans != OMX_StateIdle && dec->outputPortFlushPending != OMX_TRUE)
    {
...
//        OSAL_ThreadSleep(RETRY_INTERVAL);
        ...
        err = HantroOmx_port_lock_buffers(&dec->out);
        if (err != OMX_ErrorNone) {
            ALOGW("%s: error on call to HantroOmx_port_lock_buffer", __func__);
            return err;
        }
    ...
    if ((buff->flags & BUFFER_FLAG_MY_BUFFER) || (buff->flags & BUFFER_FLAG_NATIVE_BUFFER))
    {
    ...
    else
    {
        ...
        {
            err = grow_frame_buffer(dec, temp, framesize);
            if (err != OMX_ErrorNone)
                return OMX_ErrorInsufficientResources;
        }
...
//    static int num = 0;
...
#ifdef DECODER_COLOR_FORMAT_NV21
    ALOGD("Swap begin");
    uint8x16_t *uv = (uint8x16_t *)(frm->fb_bus_data + (frm->size / 3) * 2);
    for (i = 0; i < frm->size / 3 / sizeof(uint8x16_t); i++)
	uv[i] = vrev16q_u8(uv[i]);
    ALOGD("Swap end");
#endif
    ...
    if (frm->fb_bus_data == dec->frame_out.bus_data)
    {
        ...
        TRACE_PRINT("ASYNC: copying from temporary output buffer\n");
	if (dec->useNativeBuf) {
		struct private_handle_t *handle = (struct private_handle_t *)buff->header->pBuffer;
		memcpy(handle->base, frm->fb_bus_data, frm->size);
	}
	else
...
/*
	if (dec->useNativeBuf) {
		...
		if ((num % 30) == 0) {
			...
			sprintf(fname, "/tmp/codec%04u.bmp", num);
			ALOGD("Creating %s", fname);
			fd = open(fname, O_CREAT | O_WRONLY, (mode_t)0644);
			if (fd < 0)
				ALOGE("%s: %d (%s)", fname, errno, strerror(errno));
			else {
				write(fd, &bmfh, 14);
				write(fd, &bmih, 40);
...
*/
    ...
    {
        ALOGE("----> [TIMESTAMP_FIX] !!! no timestamp in queue !!!");
	...
	if (buff->header->nTimeStamp != 0)
	    dec->ts_buf.cur_timestamp = buff->header->nTimeStamp;
	else
	    buff->header->nTimeStamp = dec->ts_buf.cur_timestamp;
    }
    else
	buff->header->nTimeStamp = inbuff->header->nTimeStamp;
#endif
...
static void async_dispatch_last_frame(OMX_DECODER * dec)
{
    ...
    if (dec->buffer == NULL) {
	TRACE_PRINT("ASYNC: %s dec->buffer == NULL", __func__);
        return;
    }
                                   ...
                                   BUFFER * inbuff)
{
    ...
    OMX_ERRORTYPE err = OMX_ErrorNone;
        ...
        TRACE_PRINT("---------> [TIMESTAMP_FIX] added timestamp %llu at %u (inbuff size = %u)", inbuff->header->nTimeStamp, dec->timestampsQueueTail, inbuff->header->nFilledLen);
    ...
    while (state == CODEC_HAS_FRAME && !dec->outputPortFlushPending)
    {
        ...
        if (err != OMX_ErrorNone || dec->outputPortFlushPending)
            return err;
	if (frm.fb_size == 0 && frm.fb_bus_address == 0 && err == OMX_ErrorNone)
	    return OMX_ErrorNone;

        state = dec->codec->getframe(dec->codec, &frm, EOS);
        TRACE_PRINT("ASYNC: %s state %d\n", __func__, state);
    ...
    if (dec->outputPortFlushPending) {
        ...
        while (state == CODEC_HAS_FRAME) {
            ...
            TRACE_PRINT("ASYNC: %s flushing internal codec frame using buffer bus address: 0x%x", __func__, frm.fb_bus_address);
            ...
            if (state == CODEC_HAS_FRAME) {
                dec->timestampsQueueHead++;
                dec->timestampsQueueHead %= TIMESTAMPS_QUEUE_SIZE;
                TRACE_PRINT("---> [TIMESTAMP_FIX] increment head by 1 (tail = %u, head = %u)", dec->timestampsQueueTail, dec->timestampsQueueHead);
            }
#endif
                                    ...
                                    BUFFER * buff, OMX_U32 * retlen)
{
    ...
    while ( (datalen > 0) && (!dec->outputPortFlushPending) )
    {
        ...
        if (err != OMX_ErrorNone)
        {
            ...
            ALOGE("[%s] error getting frame buffer (output buffer)", __func__);
            return err;
        }
        ...
        case CODEC_NEED_MORE:
            ALOGV("streamlen = %u bytes", stream.streamlen);
            ALOGV("dec->codec->decode() returned CODEC_NEED_MORE");
            break;
        case CODEC_HAS_INFO:
        ...
        case CODEC_ERROR_STREAM:
            ...
            ALOGW("dec->codec->decode() returned CODEC_ERROR_STREAM");
        ...
        case CODEC_ERROR_STREAM_NOT_SUPPORTED:
            ALOGE("dec->codec->decode() returned CODEC_ERROR_STREAM_NOT_SUPPORTED");
            return OMX_ErrorNotImplemented;
        case CODEC_ERROR_FORMAT_NOT_SUPPORTED:
            ALOGE("dec->codec->decode() returned CODEC_ERROR_FORMAT_NOT_SUPPORTED");
            return OMX_ErrorNotImplemented;
        case CODEC_ERROR_INVALID_ARGUMENT:
            ALOGE("dec->codec->decode() returned CODEC_ERROR_INVALID_ARGUMENT");
            return OMX_ErrorBadParameter;
        default:
...
static OMX_ERRORTYPE async_decoder_decode(OMX_DECODER * dec)
{
       ...
       !(buff->flags & BUFFER_FLAG_MY_BUFFER))
    {
        ...
        TRACE_PRINT("ASYNC: copy input buffer data into the temporary input buffer");
        memcpy(dst, src, len);
        temp->size += len;
    ...
    else
    {
        ...
        if (retlen)
        {
            ...
            OMX_U8 *src = bus_data;
            TRACE_PRINT("copy data into the temporary buffer");
