                                      ...
                                      FRAME * frame)
{
    ...
    *consumed = 0;
    ...
    if (this->pp_state == PP_PIPELINE && this->update_pp_out == OMX_TRUE)
    {
        ...
        if (res != PP_OK)
        {
            ALOGE("error in line %d: HantroHwDecOmx_pp_set returned %d", __LINE__, res);
            ALOGE("pp config: input %dx%d, output %dx%d", this->pp_config.ppInImg.width, this->pp_config.ppInImg.height,
    ...
    switch (ret)
    {
        ...
        case VP8DEC_PARAM_ERROR:
            ALOGE("error in line %d: VP8DecDecode returned %d", __LINE__, ret);
...
static CODEC_STATE decoder_getinfo_vp8(CODEC_PROTOTYPE * arg, STREAM_INFO * pkg)
{
    ...
    {
        ALOGE("error in line %d: VP8DecGetInfo returned %d", __LINE__, ret);
        return CODEC_ERROR_INVALID_ARGUMENT;
    }
                                        ...
                                        OMX_BOOL eos)
{
    ...
    {
        ALOGE("error in line %d: VP8DecNextPicture returned %d", __LINE__, ret);
        return CODEC_ERROR_INVALID_ARGUMENT;
    }
                                         ...
                                         PP_ARGS * args)
{
    ...
    if (ret == PP_OK)
    {
        ...
        {
            ALOGE("error in line %d: HantroHwDecOmx_pp_set_output returned PPTR_ARG_ERROR", __LINE__);	
            return CODEC_ERROR_INVALID_ARGUMENT;
        }
#ifdef ENABLE_PP
