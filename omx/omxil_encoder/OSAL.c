...
#ifdef OMX_OSAL_TRACE

#ifdef ANDROID_MOD
#undef LOG_TAG
#define LOG_TAG "Hantro_omx_OSAL_component"
...
#else // OMX_OSAL_TRACE
#define TRACE_PRINT	fprintf_dummy
#define ALOGD		fprintf_dummy
#define ALOGE		fprintf_dummy
#endif
...
OSAL_ERRORTYPE OSAL_AllocatorInit(OSAL_ALLOCATOR* alloc)
{
    ...
    // open encoder for memory mapping
    alloc->fd_enc = open("/dev/hx280enc", O_RDWR | O_SYNC);
    if (alloc->fd_enc == -1)
    {
        TRACE_PRINT("hx280enc not found\n");
        err = OSAL_ERROR_UNDEFINED;
        goto FAIL;
    }
/*
        ...
        OSAL_U8** bus_data, OSAL_BUS_WIDTH* bus_address)
{
 ...
 //   memset(&info, 0, sizeof(DWLLinearMem_t));
    ALOGD("OSAL_AllocatorAllocMem size=%d\n",*size);
...
//    assert(alloc->fd_mem > 0);
    ...
    *bus_data   = MAP_FAILED;
    ...
    TRACE_PRINT("mmap(0, %d, %x, %x, %d, 0x%x);\n", *size, PROT_READ | PROT_WRITE, MAP_SHARED, alloc->fd_memalloc, params.busAddress);
    *bus_data = (OSAL_U8*)mmap64(0, *size, PROT_READ | PROT_WRITE, MAP_SHARED, alloc->fd_enc, params.busAddress);
    ...
    TRACE_PRINT("mmap success. vir addr = 0x%p\n",*bus_data);
...
{
#ifdef USE_DWL
	TRACE_PRINT("OSAL_AllocatorFreeMem\b");
	DWLLinearMem_t info;
	info.size = size;
	...
	TRACE_PRINT("OSAL_AllocatorFreeMem %u ok\n", bus_address);
    return;
#endif
#ifdef MEMALLOCHW
    assert(alloc->fd_memalloc > 0);
        ...
        OSAL_BOOL* pbTimedOut)
{
    ...
    events = calloc(nCount, sizeof(struct epoll_event));
    if (events == NULL)
	return ENOMEM;

    epfd = epoll_create(nCount);
    if (epfd < 0)
	return errno;
	...
	ev.events = EPOLLIN;
	ev.data.fd = pEvent->fd[0];

	if (epoll_ctl(epfd, EPOLL_CTL_ADD, pEvent->fd[0], &ev) < 0) {
	    free(events);
	    return errno;
	}
    }
	...
	n = epoll_wait(epfd, events, nCount, -1);
    else
	n = epoll_wait(epfd, events, nCount, mSecs);

    for (i = 0; i < nCount; i++) {
	OSAL_THREAD_EVENT* pEvent = (OSAL_THREAD_EVENT*)(hEvents[i]);
	if (events->data.fd == pEvent->fd[0])
            bSignaled[i] = 1;
        else
            bSignaled[i] = 0;
    }
