...
ALOGV("%s: H264EncStrmEncode start", __func__);
    if (ret == H264ENC_OK)
#if !defined (ENC8290) && !defined (ENCH1)
...
ENCODER_PROTOTYPE* HantroHwEncOmx_encoder_create_h264(const H264_CONFIG* params)
{
    ...
    if (params->h264_config.eProfile != OMX_VIDEO_AVCProfileBaseline)
    {
		ALOGE("[%s] only baseline profile is supported (requested profile: %d)", __FUNCTION__, params->h264_config.eProfile);
        return NULL;
    }
#endif
...
#endif
        default:
	    ALOGE("[%s] unsupported encoding level %d", __FUNCTION__, params->h264_config.eLevel);
            return NULL;
            break;
    }
    ...
    if (ret == H264ENC_OK)
    {
        ...
        ALOGE("[%s] H264EncInit failed! (%d)", __func__, ret);
        OSAL_Free(this);
        return NULL;
    }
    ...
    if (ret == H264ENC_OK)
    {
        ...
        if (ret == H264ENC_OK)
        {
            ...
            if (params->rate_config.nQpMin > 0)
            {
                ...
                if(rate_ctrl.qpHdr < (i32)rate_ctrl.qpMin)
                {
            ...
            if (params->rate_config.nQpMax > 0)
            {
                ...
                if(rate_ctrl.qpHdr > (i32)rate_ctrl.qpMax)
                {
            ...
            ALOGE("[%s] H264EncGetRateCtrl failed! (%d)", __func__, ret);
            OSAL_Free(this);
            return NULL;
        }
    }
    ...
    if (ret == H264ENC_OK)
    {
...
#endif
#if defined (ENC7280) || defined (ENC8270) || defined (ENC8290) || defined (ENCH1)
                pp_config.inputType = H264ENC_YUV420_SEMIPLANAR_VU;
#endif
                break;
        ...
        ALOGE("[%s] H264EncSetRateCtrl failed! (%d)", __func__, ret);
        OSAL_Free(this);
        return NULL;
    }
    ...
    if (ret != H264ENC_OK)
    {
        ALOGE("[%s] ret != H264ENC_OK (%d)", __FUNCTION__, ret);
        OSAL_Free(this);
        return NULL;
    }
