...
#ifdef ANDROID
//#define LOG_NDEBUG 0
#define LOG_TAG "omxil_8290_encoder_component"
#define __USE_GNU 1
#include "yuv.h"
#undef __USE_GNU
#include <log/log.h>
#define TRACE_PRINT(args...) ALOGV(args)
        ...
        if (signals[0] == OMX_TRUE)
        {
            ...
            while (1)
            {
                ...
                if (cmd.arg.cmd == OMX_CommandFlush) {
                    ...
                    TRACE_PRINT("Entering into FLUSHING STATE");
                } else if (cmd.arg.cmd == OMX_CommandPortEnable) {
                    bInFlushState = OMX_FALSE;
                    TRACE_PRINT("Coming out of FLUSHING STATE");
                }
...
OMX_ERRORTYPE encoder_deinit( OMX_HANDLETYPE hComponent )
{
    ...
    if (pEnc->state != OMX_StateLoaded)
    {
        ...
        if (pEnc->tempBuffer.bus_address)
            FRAME_BUFF_FREE(&pEnc->alloc, &pEnc->tempBuffer);
    }
    else
    {
	...
	TRACE_PRINT("%s: almost done", __FUNCTION__);
                                 ...
                                 OMX_IN    OMX_U8* pBuffer)
{
    ...
    struct VideoGrallocMetadata *Metadata;
    ...
    if (!port->StoreMetadata) {
	err = encoder_verify_buffer_allocation(pEnc, port, nSizeBytes);
	if (err != OMX_ErrorNone)
	    return err;
    }
                                      ...
                                      OMX_IN    OMX_U32 nSizeBytes)
{
    ...
    if (err != OMX_ErrorNone)
        return err;
ALOGV("%s: alocsize = %u bus_data = %p bus_address = 0x%x", __func__, allocsize, bus_data, bus_address);
...
//    TRACE_PRINT("API: data (virtual address):%p pBufferHeader:%p\n", bus_data, *ppBuffer);
    ALOGV("%s: bus_data = %p bus_address = %u allocsize = %u pBuffer = %p nAllocLen = %u", __func__,
	    buff->bus_data, buff->bus_address, buff->allocsize, buff->header->pBuffer, buff->header->nAllocLen);
                                    ...
                                    OMX_IN OMX_PTR        pParam)
{
    ...
    switch (nIndex)
    {
		...
		TRACE_PRINT("%s: OMX_IndexParamPortDefinition: port = %u nBufferCountActual = %d nBufferSize = %d eColorFormat = %d nFrameWidth = %d nFrameHeight = %d",
			__func__, param->nPortIndex, param->nBufferCountActual, param->nBufferSize, param->format.video.eColorFormat, param->format.video.nFrameWidth, param->format.video.nFrameHeight);
...
//                        port->format.video.nFrameWidth        = param->format.video.nFrameHeight;
//                        port->format.video.nFrameHeight       = param->format.video.nFrameWidth;
                        port->format.video.nStride            = param->format.video.nStride;
//                        port->format.video.nStride            = param->format.video.nFrameHeight * 4;
...
//                        port->format.video.nFrameHeight       = param->format.video.nFrameWidth;
		...
		TRACE_PRINT("OMX_IndexParamVideoPortFormat: port = %u, eColorFormat = %u", param->nPortIndex, param->eColorFormat);
                switch (param->nPortIndex)
                {
                    case PORT_INDEX_INPUT:
                        port = &pEnc->inputPort.def;
        ...
        case OMX_IndexParamVideoAvc:
            {
                ...
                {
                    case PORT_INDEX_INPUT:
                        TRACE_PRINT("No H264 config for input.\n");
                        break;
                    case PORT_INDEX_OUTPUT:
                        port = &pEnc->outputPort.def;
                        CHECK_PORT_STATE(pEnc, port );
        ...
        case OMX_IndexParamVideoH263: //OMX_VIDEO_PARAM_H263TYPE
            {
                ...
                {
                    case PORT_INDEX_INPUT:
                        TRACE_PRINT("No H263 config for input.\n");
                        break;
                    case PORT_INDEX_OUTPUT:
                        port = &pEnc->outputPort.def;
                        CHECK_PORT_STATE(pEnc, port );
        ...
        case OMX_IndexParamVideoMpeg4: //OMX_VIDEO_PARAM_MPEG4TYPE
            {
                ...
                {
                    case PORT_INDEX_INPUT:
                            TRACE_PRINT("No mpeg4 config for input.\n");
                            break;
                    case PORT_INDEX_OUTPUT:
                        port = &pEnc->outputPort.def;
                        CHECK_PORT_STATE(pEnc, port );
        ...
        case OMX_IndexParamVideoVp8:
            {
                ...
                {
                    case PORT_INDEX_INPUT:
                        TRACE_PRINT("No VP8 config for input.\n");
                        break;
                    case PORT_INDEX_OUTPUT:
                        port = &pEnc->outputPort.def;
                        CHECK_PORT_STATE(pEnc, port );
       ...
       case OMX_IndexParamCompBufferSupplier:
        ...
        case OMX_IndexParamVideoBitrate:
        {
            ...
            if ( paramBitrate->nPortIndex == PORT_INDEX_OUTPUT )
            {
                ...
                if ( paramBitrate->nTargetBitrate != pEnc->outputPort.def.format.video.nBitrate )
                {
					...
					TRACE_PRINT("before PortSettingsChanged event: %d",__LINE__);
#ifdef ANDROID_MOD
        ...
        case OMX_IndexParamVideoQuantization:
            {
                TRACE_PRINT("API: got VideoQuantization param\n");
                ...
                if ( paramQ->nPortIndex == PORT_INDEX_OUTPUT )
                {
                    ...
                    TRACE_PRINT("API:  QpI value: %d\n", (int)portQ->nQpI);
                    TRACE_PRINT("API:  QpP value: %d\n", (int)portQ->nQpP);
                }
	...
	{
		struct StoreMetaDataInBuffersParams *param = (struct StoreMetaDataInBuffersParams *)pParam;
		TRACE_PRINT("OMX.google.android.index.storeMetaDataInBuffers: port = %d StoreMetadata = %d", param->nPortIndex, param->bStoreMetaData);
		switch (param->nPortIndex) {
		    case PORT_INDEX_INPUT:
			pEnc->inputPort.StoreMetadata = param->bStoreMetaData;
			break;
/*
		    case PORT_INDEX_OUTPUT:
			pEnc->outputPort.StoreMetadata = param->bStoreMetaData;
			break;
*/
		    default:
			ALOGE("OMX.google.android.index.storeMetaDataInBuffers: invalid port index %u", param->nPortIndex);
			return OMX_ErrorUnsupportedIndex;
		}
		break;
	}
...
#endif
#ifdef ANDROID_MOD
        { OMX_VIDEO_CodingUnused, OMX_COLOR_FormatAndroidOpaque },
#endif
...
/*    static int avc_profiles[][2] = {
    ...
    switch (nIndex)
    {
        ...
        case OMX_IndexParamPortDefinition:
            {
                ...
                switch (param->nPortIndex)
                {
                    case PORT_INDEX_INPUT:
                        port = &pEnc->inputPort.def;
			TRACE_PRINT("requested input port definition. port->nBufferSize = %d", port->nBufferSize);
                        break;
                    case PORT_INDEX_OUTPUT:
                        port = &pEnc->outputPort.def;
                        break;
		...
		TRACE_PRINT("OMX_IndexParamPortDefinition: port = %u eColorFormat = %u", param->nPortIndex, port->format.video.eColorFormat);
            }
            break;
        case OMX_IndexParamVideoPortFormat:
            {
                ...
                {
                    case PORT_INDEX_INPUT:
                        TRACE_PRINT("OMX_IndexParamVideoPortFormat (input:%d)", param->nIndex);
                        if (param->nIndex >= sizeof(input_formats)/(sizeof(int)*2)) {
                            TRACE_PRINT("returning OMX_ErrorNoMore");
                            return OMX_ErrorNoMore;
                        }
                        param->xFramerate = 0;
                ...
                switch ( pEnc->outputPort.def.format.video.eCompressionFormat )
                {
                    case OMX_VIDEO_CodingAVC:
                        if (param->nProfileIndex >= sizeof(avc_profiles)/(sizeof(int)*2)) {
							TRACE_PRINT("error: param->nProfileIndex >= sizeof(avc_profiles)/(sizeof(int)*2)");
                            return OMX_ErrorNoMore;
						}
                        param->eProfile = avc_profiles[param->nProfileIndex][0];
                        param->eLevel   = avc_profiles[param->nProfileIndex][1];
						TRACE_PRINT("param->eProfile = %d, param->eLevel = %d", param->eProfile, param->eLevel);
                        break;
#if !defined (ENC8270) && !defined (ENC8290) && !defined (ENCH1)
                    case OMX_VIDEO_CodingH263:
	...
	TRACE_PRINT("[%s] return ok",__FUNCTION__);
    return OMX_ErrorNone;
}
                                    ...
                                    OMX_IN OMX_PTR        pParam)
{
    ...
    switch (nIndex)
    {
        ...
        case OMX_IndexParamPortDefinition:
            {
                ...
                {
                    TRACE_PRINT("API: unsupported port domain\n");
                    ...
                    case PORT_INDEX_INPUT:
                        ...
                        switch (param->format.image.eColorFormat)
                        {
                            ...
                            case OMX_COLOR_Format32bitARGB8888:
                                ...
                                break;
                            default:
                                TRACE_PRINT("API: unsupported image color format\n");
                                return OMX_ErrorUnsupportedSetting;
                        }
                        if ( param->nBufferCountActual >= port->nBufferCountMin )
                            port->nBufferCountActual              = param->nBufferCountActual;
        ...
        {
            TRACE_PRINT("API: OMX_IndexParamQFactor\n");
            ...
            if ( paramQf->nPortIndex == PORT_INDEX_OUTPUT )
            {
                ...
                TRACE_PRINT("API: QFactor: %d\n", (int)portQf->nQFactor);
            }
            else
            {
                TRACE_PRINT("API: No such port: %d\n", (int)paramQf->nPortIndex);
                return OMX_ErrorBadPortIndex;
            }
        }
        break;
        ...
        case OMX_IndexConfigCommonInputCrop:
            {
                ...
                //height and width.
                TRACE_PRINT("SetConfig Crop, nLeft : %u\n", (unsigned)param->nLeft );
                TRACE_PRINT("SetConfig Crop, nTop : %u\n", (unsigned)param->nTop );
...
#endif
                TRACE_PRINT("SetConfig Crop, port->format.image.nFrameHeight : %u\n", (unsigned)port->format.image.nFrameHeight );
                TRACE_PRINT("SetConfig Crop, port->format.image.nFrameWidth : %u\n", (unsigned)port->format.image.nFrameWidth );
                calculate_new_outputBufferSize( pEnc );
        ...
        case OMX_IndexConfigCommonFrameStabilisation:
            {
                ...
                if ( param->nPortIndex == PORT_INDEX_OUTPUT )
                {
                    TRACE_PRINT("API: wrong port index\n");
                    return OMX_ErrorBadPortIndex;
                }
        ...
        case OMX_IndexConfigVideoBitrate:
            {
				...
				TRACE_PRINT("encoder_set_config: OMX_IndexConfigVideoBitrate: %d", param->nEncodeBitrate);
				...
				TRACE_PRINT("encoder_set_config: OMX_IndexConfigVideoBitrate: done");
                                  ...
                                  OMX_U32 portindex)
{
    ...
    {
        TRACE_PRINT("API: incorrect encoder state: %s\n", HantroOmx_str_omx_state(pEnc->state));
        return OMX_ErrorIncorrectStateOperation;
    }
...
OMX_ERRORTYPE encoder_get_extension_index(OMX_HANDLETYPE hComponent, OMX_STRING cParameterName, OMX_INDEXTYPE* pIndexType)
{
	...
	{
		*pIndexType = (OMX_INDEXTYPE)OMX_GoogleIndexStoreMetaDataInBuffers;
	}
	else
	{
		ALOGE("%s: %s not implemented!\n", __func__, cParameterName);
		return OMX_ErrorNotImplemented;
	}
#else
	return OMX_ErrorNotImplemented;
...
OMX_ERRORTYPE HantroHwEncOmx_encoder_init( OMX_HANDLETYPE hComponent )
{
	...
	OMX_ERRORTYPE err = HantroOmx_port_init(&pEnc->inputPort, 4, 8, 8, defaultWidth*defaultHeight*3/2);
    if (err != OMX_ErrorNone)
        goto INIT_FAILURE;
    ...
    err = HantroOmx_port_init(&pEnc->outputPort, 4, 8, 8, outputBufferSize);
    if (err != OMX_ErrorNone)
        goto INIT_FAILURE;
...
//    pEnc->inputPort.def.format.video.eColorFormat       = OMX_COLOR_FormatYUV420PackedSemiPlanar;
    pEnc->inputPort.def.format.video.eColorFormat       = OMX_COLOR_FormatYUV420SemiPlanar;
#else
    ...
    pEnc->outputPort.def.format.video.nBitrate              = 8000000;
...
//    pEnc->outputPort.def.format.video.eColorFormat          = OMX_COLOR_FormatUnused;
    pEnc->outputPort.def.format.video.eColorFormat          = OMX_COLOR_FormatYCbYCr;
	...
	case OMX_COLOR_FormatAndroidOpaque:
        ...
        case OMX_COLOR_FormatYCbYCr: // YCbCr 4:2:2 interleaved (YUYV)
            *frameSize    =
            ...
            break;
#if defined (ENCH1) || defined (ENC8290)
        case OMX_COLOR_Format24bitRGB888:
        case OMX_COLOR_Format24bitBGR888:
        case OMX_COLOR_Format25bitARGB1888:
        case OMX_COLOR_Format32bitARGB8888:
...
#endif
        default:
            TRACE_PRINT("ASYNC: unsupported format\n");
            return OMX_ErrorUnsupportedIndex;
            break;
    }
    return OMX_ErrorNone;
...
OMX_ERRORTYPE set_avc_defaults( OMX_ENCODER* pEnc)
{
    ...
    config->eLevel = OMX_VIDEO_AVCLevel41;
    config->eLoopFilterMode = OMX_VIDEO_AVCLoopFilterEnable;
    config->nRefFrames = 1;
	TRACE_PRINT("1)--> config->nAllowedPictureTypes = %d", config->nAllowedPictureTypes);
#if 0//def ANDROID_MOD
	config->nAllowedPictureTypes = OMX_VIDEO_PictureTypeI | OMX_VIDEO_PictureTypeP;
#else
    config->nAllowedPictureTypes &= OMX_VIDEO_PictureTypeI | OMX_VIDEO_PictureTypeP;
#endif
	TRACE_PRINT("2)--> config->nAllowedPictureTypes = %d", config->nAllowedPictureTypes);
    config->nPFrames = 150;
...
{
    TRACE_PRINT("ASYNC: %s\n", __FUNCTION__);
    TRACE_PRINT("ASYNC: pEnc->sliceNum - %u \n", (unsigned)pEnc->sliceNum);
    TRACE_PRINT("ASYNC: pEnc->numOfSlices - %u \n", (unsigned)pEnc->numOfSlices);
        ...
        case OMX_COLOR_FormatYUV420PackedPlanar:
            ...
            else
            {
                ...
                else
                {
                    ...
                    TRACE_PRINT("ASYNC: Encoding last slice...\n");
 ...
 */
 ...
 static OMX_ERRORTYPE calculate_new_outputBufferSize( OMX_ENCODER* pEnc )
 {
    ...
    TRACE_PRINT("API: framewidth - %u\n", (unsigned)frameWidth);
    TRACE_PRINT("API: frameHeight - %u\n", (unsigned)frameHeight);
    ...
    TRACE_PRINT("API: calculated new buffer size - %u\n", (unsigned)newBufferSize);
    TRACE_PRINT("API: portDef->nBufferSize - %u\n", (unsigned)portDef->nBufferSize);
    return OMX_ErrorNone;
 }
...
//    if (pEnc->inputPort.def.format.video.nStride & 0x0f)
    if (pEnc->inputPort.def.format.video.nStride & 0x03)
    {
	ALOGE("input video Stride = %u, %u & 0x0f != 0!", pEnc->inputPort.def.format.video.nStride, pEnc->inputPort.def.format.video.nStride);
...
TRACE_PRINT("TEST Codec\n");
    ...
    switch ( pEnc->outputPort.def.format.video.eCompressionFormat )
    {
        case OMX_VIDEO_CodingAVC:
            {
            TRACE_PRINT("ASYNC: Creating H264 codec...\n");
            H264_CONFIG config;
	    ...
	    if (pEnc->inputPort.def.format.video.eColorFormat == OMX_COLOR_FormatAndroidOpaque)
		config.pp_config.formatType = OMX_COLOR_FormatYUV420Planar;
	    else
		config.pp_config.formatType = pEnc->inputPort.def.format.video.eColorFormat;
                 ...
                 pEnc->outputPort.def.format.video.nFrameHeight == pEnc->inputPort.def.format.video.nFrameHeight )
            {
                ...
                TRACE_PRINT("ASYNC: Using default stabilization values \n");
                config.pp_config.angle = pEnc->encConfig.rotation.nRotation;
            ...
            else if ( config.pp_config.frameStabilization )
            {
                ...
                //No other crop
                TRACE_PRINT("ASYNC: Using user defined stabilization values \n");
                config.pp_config.xOffset = 0;
                config.pp_config.yOffset = 0;
            }
            else
            {
                TRACE_PRINT("ASYNC: No stabilization \n");
            ...
            switch ( config.rate_config.eRateControl )
            {
                ...
                default:
                {
                    TRACE_PRINT("No rate control defined...using disabled value\n");
...
ALOGV("%s: codec created", __func__);
        }
        break;
#if !defined (ENC8270) && !defined (ENC8290) && !defined (ENCH1)
    case OMX_VIDEO_CodingMPEG4:
    {
        TRACE_PRINT("ASYNC: Creating MPEG4 codec\n");

        MPEG4_CONFIG config;

        config.pp_config.origWidth = pEnc->inputPort.def.format.video.nStride;
        TRACE_PRINT("ASYNC: config.pp_config.origWidth - %u\n", (unsigned) config.pp_config.origWidth);
        config.pp_config.origHeight =  pEnc->inputPort.def.format.video.nFrameHeight;
        TRACE_PRINT("ASYNC: config.pp_config.origHeight - %u\n", (unsigned)config.pp_config.origHeight);
        config.common_config.nInputFramerate = pEnc->inputPort.def.format.video.xFramerate;
        TRACE_PRINT("ASYNC: config.common_config.nInputFramerate - %u\n", (unsigned) config.common_config.nInputFramerate);
        config.pp_config.formatType = pEnc->inputPort.def.format.video.eColorFormat;
        TRACE_PRINT("ASYNC: config.pp_config.formatType - %u\n", (unsigned)config.pp_config.formatType);
        config.pp_config.frameStabilization = pEnc->encConfig.stab.bStab;
        TRACE_PRINT("ASYNC: config.pp_config.frameStabilization - %u\n", (unsigned) config.pp_config.frameStabilization);
        config.pp_config.angle = pEnc->encConfig.rotation.nRotation;
#if 1
             if ( pEnc->encConfig.rotation.nRotation == 90 || pEnc->encConfig.rotation.nRotation == 270 )
            {
        ...
        switch ( config.rate_config.eRateControl )
        {
            case OMX_Video_ControlRateVariable:
            {
                ...
                config.rate_config.nHrdEnabled = 0;
                config.rate_config.nPictureRcEnabled = 1;
                                TRACE_PRINT("ASYNC: eRateControl: OMX_VIDEO_ControlRateVariable\n");
            }
            break;
            case OMX_Video_ControlRateDisable:
            {
                ...
                config.rate_config.nHrdEnabled = 0;
                config.rate_config.nPictureRcEnabled = 0;
                                TRACE_PRINT("ASYNC: eRateControl: OMX_VIDEO_ControlRateDisable\n");
            }
            break;
            case OMX_Video_ControlRateConstant:
            {
                ...
                config.rate_config.nHrdEnabled = 1;
                config.rate_config.nPictureRcEnabled = 1;
                                TRACE_PRINT("ASYNC: eRateControl: OMX_VIDEO_ControlRateConstant\n");
            }
            break;
            case OMX_Video_ControlRateConstantSkipFrames:
            {
                ...
                config.rate_config.nHrdEnabled = 1;
                config.rate_config.nPictureRcEnabled = 1;
                                TRACE_PRINT("ASYNC: eRateControl: OMX_VIDEO_ControlRateConstantSkipFrames\n");
            }
            break;
            case  OMX_Video_ControlRateVariableSkipFrames:
            {
                ...
                config.rate_config.nHrdEnabled = 0;
                config.rate_config.nPictureRcEnabled = 1;
                                TRACE_PRINT("ASYNC: eRateControl: OMX_VIDEO_ControlRateVariableSkipFrames\n");
            }
            break;
            default:
            {
        ...
        TRACE_PRINT("ASYNC: config.rate_config.eRateControl - %u\n", (unsigned) config.rate_config.eRateControl );
        TRACE_PRINT("ASYNC: config.rate_config.nMbRcEnabled - %u\n", (unsigned) config.rate_config.nMbRcEnabled );
        TRACE_PRINT("ASYNC: config.rate_config.nHrdEnabled - %u\n", (unsigned) config.rate_config.nHrdEnabled );
        TRACE_PRINT("ASYNC: config.rate_config.nPictureRcEnabled - %u\n", (unsigned) config.rate_config.nPictureRcEnabled );
//        config.rate_config.nVbvEnabled = 1; // default, enabled. Should be always 1 (ON)
    ...
    case OMX_VIDEO_CodingH263:
    {
        TRACE_PRINT("ASYNC: Creating H263 codec\n");
        ...
        case OMX_VIDEO_CodingVP8:
        {
            TRACE_PRINT("ASYNC: Creating VP8 codec...\n");
            VP8_CONFIG config;
...
#endif //ENCH1
    default:
        TRACE_PRINT("ASYNC: unsupported input format. no such codec\n");
        err = OMX_ErrorUnsupportedSetting;;
        break;
    }
#endif //~OMX_ENCODER_VIDEO_DOMAIN
    ...
    {
        TRACE_PRINT("IMAGE Domain\n");
        case OMX_IMAGE_CodingJPEG:
        {
            TRACE_PRINT("Creating JPEG encoder\n");
            ...
            TRACE_PRINT("ASYNC: config.qLevel - %u\n", (unsigned) config.qLevel );
            ...
            {
                TRACE_PRINT("config.codingType = JPEGENC_WHOLE_FRAME\n" );
                config.codingType = JPEGENC_WHOLE_FRAME;
            }
            else
            {
                TRACE_PRINT("config.codingType = JPEGENC_SLICED_FRAME;\n" );
                config.codingType = JPEGENC_SLICED_FRAME;
                pEnc->sliceMode = OMX_TRUE;
                if ( config.codingHeight % config.sliceHeight == 0 )
                {
                    TRACE_PRINT("config.codingHeight config.sliceHeight > 0 - TRUE \n" );
                    pEnc->numOfSlices = config.codingHeight / config.sliceHeight;
                }
                else
                {
                    TRACE_PRINT("config.codingHeight config.sliceHeight > 0 - FALSE \n" );
        ...
        case OMX_IMAGE_CodingWEBP:
        {
            TRACE_PRINT("Creating WEBP encoder\n");
            ...
            /* not supported
                ...
                config.sliceHeight == 0 )
            {
                TRACE_PRINT("config.codingType = WEBPENC_WHOLE_FRAME\n" );
                config.codingType = WEBPENC_WHOLE_FRAME;
            }
            else
            {
                TRACE_PRINT("config.codingType = WEBPENC_SLICED_FRAME;\n" );
    ...
    if ( !codec )
    {
        TRACE_PRINT("ASYNC: Codec creating error\n");
        goto FAIL;
    }
...
//        goto FAIL;
	...
	TRACE_PRINT("calling OMX_AllocatorAllocMem with size = %d",output_buffer_size);
    err = OSAL_AllocatorAllocMem(&pEnc->alloc, &output_buffer_size, &out.bus_data, &out.bus_address);
    if (err != OMX_ErrorNone)
        goto FAIL;
	TRACE_PRINT("calling OMX_AllocatorAllocMem success with address = 0x%x",out.bus_address);
...
#ifdef ANDROID
static void power_module_hint(OMX_BOOL executing)
{
    power_module_t *power_module;

    if (hw_get_module(POWER_HARDWARE_MODULE_ID, (const hw_module_t **)&power_module)) {
        ALOGE("failed to get power hw module");
        return;
    }

    if (power_module->powerHint)
        power_module->powerHint(power_module, POWER_HINT_VIDEO_ENCODE, (void *)&executing);
}
#endif
...
#ifdef ANDROID
    if (pEnc->inputPort.def.format.video.eColorFormat == OMX_COLOR_FormatAndroidOpaque) {
	ALOGV("POWER_HINT_VIDEO_ENCODE 0");
	power_module_hint(OMX_FALSE);
    }
#endif
...
//BS:: do not free input buffer, address is used as zero copy from upper layers
    ...
    memset(&pEnc->tempBuffer, 0, sizeof(FRAME_BUFFER));
...
#ifdef ANDROID
    if (pEnc->inputPort.def.format.video.eColorFormat == OMX_COLOR_FormatAndroidOpaque) {
	ALOGV("POWER_HINT_VIDEO_ENCODE 1");
	power_module_hint(OMX_TRUE);
    }
#endif
                                        ...
                                        OMX_PTR pCmdData, OMX_PTR arg)
{
    ...
    if (portIndex == PORT_INDEX_OUTPUT)
    {
        if (pEnc->state != OMX_StateLoaded)
        {
            if (HantroOmx_port_is_supplier(&pEnc->outputPort))
...
{
    TRACE_PRINT("ASYNC: %s\n", __FUNCTION__);
    TRACE_PRINT("ASYNC: fb capacity: %u new capacity:%u\n",
...
//			usleep(5000);
...
#ifdef USE_TEMP_OUTPUT_BUFFER
    TRACE_PRINT("----> %s: tempBuffer->bus_address = 0x%x, tempBuffer->bus_data = 0x%p", __func__, tempBuffer->bus_address, tempBuffer->bus_data);
    ...
    ALOGW("---> zero copy: max_size = %d, bus_data = %p, bus_address = 0x%x", outputStream.buf_max_size, outputStream.bus_data, outputStream.bus_address);
#endif
...
OMX_ERRORTYPE async_video_encoder_encode(OMX_ENCODER* pEnc)
{
    ...
    TRACE_PRINT("ASYNC: Calculated frameSize - %u ",(unsigned)frameSize );
	...
	if (!pEnc->tempBuffer.bus_address) {
	    OMX_U8* bus_data = NULL;
	    OSAL_BUS_WIDTH bus_address = 0;
	    OMX_U32 allocsize = frameSize;

	    ALOGV("Allocating temp input buffer, allocsize = %u", allocsize);
	    err = OSAL_AllocatorAllocMem(&pEnc->alloc, &allocsize, &bus_data, &bus_address);
	    if (err != OMX_ErrorNone)
		return err;
	    ALOGV("Alllocated: %u", allocsize);
	    pEnc->tempBuffer.bus_address = bus_address;
	    pEnc->tempBuffer.bus_data = bus_data;
	    pEnc->tempBuffer.capacity = allocsize;
	}
	tempBuffer = &pEnc->tempBuffer;
	OMX_U32 width = pEnc->inputPort.def.format.video.nFrameWidth;
	OMX_U32 height = pEnc->inputPort.def.format.video.nFrameHeight;
	ALOGV("Converting to I420...");
	err = ABGRToI420_multithread(inputBuffer->bus_data, width * 4,
               tempBuffer->bus_data, width,
               tempBuffer->bus_data + (width * height), width / 2,
               tempBuffer->bus_data + (width * height * 5) / 4, width / 2,
               width, height);
	ALOGV("ABGRToI420 returned %d", err);
    }
    else {
...
//    tempBuffer->bus_data = inputBuffer->header->pBuffer + inputBuffer->header->nOffset;
    tempBuffer->bus_address = inputBuffer->bus_address + inputBuffer->header->nOffset;
    tempBuffer->bus_data = inputBuffer->bus_data + inputBuffer->header->nOffset;
}
    ...
    {
        TRACE_PRINT("ASYNC: Using internal buffer\n");
...
//        memcpy(dst, src, len);
        ...
        if ( pEnc->encConfig.stab.bStab )
        {
            if ( tempBuffer->size >= frameSize *2 )
            {
                ...
                if (err != OMX_ErrorNone)
                {
                    ...
                    {
                        TRACE_PRINT("ASYNC: firing OMX_ErrorBadParameter\n");
        ...
        {
           TRACE_PRINT("tempBuffer->bus_data = 0x%p, tempBuffer->bus_address = 0x%x, tempBuffer->size = %d, frameSize = %d\n",
    ...
    {
        TRACE_PRINT("ASYNC: No internal buffer\n");
...
EOS:
    if (inputBuffer->header->nFlags & OMX_BUFFERFLAG_EOS)
    {
        TRACE_PRINT("ASYNC: got buffer EOS flag\n");
                                ...
                                OMX_U32 datalen, BUFFER* inputBuffer, OMX_U32* retlen, OMX_U32 frameSize )
{
    ...
    while (datalen >= frameSize*2 )
    {
        ...
        switch ( pEnc->outputPort.def.format.video.eCompressionFormat )
        {
            case OMX_VIDEO_CodingAVC:
                {
                    TRACE_PRINT("ASYNC: pEnc->encConfig.avc.nPFrames - %d\n", (unsigned) pEnc->encConfig.avc.nPFrames);
            ...
            case OMX_VIDEO_CodingMPEG4:
                {
                    TRACE_PRINT("ASYNC: pEnc->encConfig.mpeg4.nPFrames - %d\n", (unsigned) pEnc->encConfig.mpeg4.nPFrames);
...
#endif
            default:
                TRACE_PRINT("Illegal compression format..using 0 at nPFrames\n");
            break;
        }
        ...
        if ( codecState < 0 )
        {
            ...
            {
                ALOGE("pEnc->codec->encode() returned CODEC_ERROR_BUFFER_OVERFLOW!");
                return OMX_ErrorOverflow;
            }
            else if (codecState == CODEC_ERROR_INVALID_ARGUMENT)
            {
                ALOGE("pEnc->codec->encode() returned CODEC_ERROR_INVALID_ARGUMENT!");
                return OMX_ErrorBadParameter;
            }
            else
            {
                ALOGE("pEnc->codec->encode() returned undefned error!");
        ...
        TRACE_PRINT("ASYNC: output buffer size: %d\n", (int)outputBuffer->header->nAllocLen);
        if (stream.streamlen > outputBuffer->header->nAllocLen)
        {
            TRACE_PRINT("ASYNC: output buffer is too small!!\n");
            return OMX_ErrorOverflow;
        }
        ...
        TRACE_PRINT("ASYNC: output buffer OK\n");
...
//		int retry = 4000;
		...
		while (outputBuffer == NULL && retry > 0) {
			...
			if( pEnc->statetrans == OMX_StateIdle){
	            TRACE_PRINT("%s: transition to idle, exit outputBuffer loop",__FUNCTION__);
				return	OMX_ErrorNone;
			}
			retry--;
			err = HantroOmx_port_lock_buffers(&pEnc->outputPort);
...
//				usleep(5000);
        ...
        switch ( pEnc->outputPort.def.format.video.eCompressionFormat )
        {
            case OMX_VIDEO_CodingAVC:
                {
                    TRACE_PRINT("ASYNC: pEnc->encConfig.avc.nPFrames - %u\n", (unsigned)pEnc->encConfig.avc.nPFrames);
...
#warning "!!! Always code intra frames !!!"
            ...
            case OMX_VIDEO_CodingMPEG4:
                {
                    TRACE_PRINT("ASYNC: pEnc->encConfig.mpeg4.nPFrames - %u\n", (unsigned) pEnc->encConfig.mpeg4.nPFrames);
                    if ( pEnc->encConfig.mpeg4.nPFrames == 0)
                        frame.frame_type = INTRA_FRAME;
                    else
                    frame.frame_type = PREDICTED_FRAME;
        ...
        if ( codecState < 0 )
        {
            TRACE_PRINT("ASYNC: codecState - %d\n", codecState);
            if (codecState == CODEC_ERROR_BUFFER_OVERFLOW)
            {
                ALOGE("pEnc->codec->encode returned CODEC_ERROR_BUFFER_OVERFLOW!");
                return OMX_ErrorOverflow;
            ...
            {
                ALOGE("pEnc->codec->encode returned undefined error!");
                return OMX_ErrorUndefined;
            }
        }
        TRACE_PRINT("ASYNC: codecState = %u\n", (unsigned)codecState);

        OMX_U32 rem = datalen - frame.fb_frameSize;

        TRACE_PRINT("ASYNC: rem = %u\n", (unsigned)rem);

        memmove(bus_data, bus_data + frame.fb_frameSize, rem );
        datalen -=  frame.fb_frameSize;

        TRACE_PRINT("ASYNC: v = %u\n", (unsigned)datalen);
        TRACE_PRINT("ASYNC: output buffer size: %d\n", (int)outputBuffer->header->nAllocLen);
        if (stream.streamlen > outputBuffer->header->nAllocLen)
        {
            TRACE_PRINT("ASYNC: output buffer is too small!\n");
...
OMX_ERRORTYPE async_image_encoder_encode(OMX_ENCODER* pEnc)
{
    ...
    err = calculate_frame_size( pEnc, &frameSize );
    TRACE_PRINT("ASYNC:  calculated frameSize - %u\n", (unsigned)frameSize );
                                ...
                                OMX_U32 datalen, BUFFER* inputBuffer, OMX_U32* retlen, OMX_U32 frameSize )
{
    ...
    while ( datalen >= frameSize )
    {
        ...
        TRACE_PRINT("ASYNC: stream.streamlen - %u \n", (unsigned)stream.streamlen);
        TRACE_PRINT("ASYNC: rem - %u \n", (unsigned)rem);

        memmove(bus_data, bus_data + frame.fb_frameSize, rem );
        datalen -=  frame.fb_frameSize;
        TRACE_PRINT("ASYNC: bytes left in temp buffer - %u \n",(unsigned)datalen );
        ...
        if ( pEnc-> sliceMode )
        {
            TRACE_PRINT("ASYNC: pEnc->sliceCounter %u\n", (unsigned)pEnc->sliceNum);
            TRACE_PRINT("ASYNC: pEnc->numOfSlices %u\n", (unsigned)pEnc->numOfSlices );
            pEnc->sliceNum++;
            if ( pEnc->sliceNum > pEnc->numOfSlices )
            {
                pEnc->frameCounter++;
                TRACE_PRINT("OMX_BUFFERFLAG_ENDOFFRAME \n");
